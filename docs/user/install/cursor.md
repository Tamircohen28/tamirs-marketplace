...
| | |
|---|---|
| **Validated against** | Cursor **3.20.17** |
| **Minimum supported** | **3.20.17** |
| **Changelog covered through** | feature **3.11** + date-only entries to **2026-09-10** (see [`.cursor-version`](../../../.cursor-version)) |
...
Cursor's documentation states **no minimum version** for plugins. The 3.20.17 floor here is
...
## Working tips (3.11 → 2026-09-10; desktop CLI 3.20.17; Grok 4.6)
...
- **Desktop CLI patch line** — pin is **3.20.17** (download line 2026-08-18; [CLI changelog](https://cursor.com/docs/cli/changelog) Aug 11). Newest feature write-up remains **3.11**; newest date-only entry **2026-08-19** (cloud-agent subscriptions / custom modes / isolated subagent VMs / `/goal` / steering).
...
Cursor **Projects** (left nav) are for larger, longer-running bodies of work. A **coordinator agent** plans and delegates to many agents and does not write code itself. Shared context files sync across every cloud and local machine the Project uses. Subscriptions can watch Slack, run on a schedule, or follow all PRs. This catalog still installs via the Cursor Dashboard marketplace import — Projects do not change the import path. Prefer a Project when rolling out plugin updates across many repos over weeks; keep one-off marketplace edits in a normal Agent chat.
