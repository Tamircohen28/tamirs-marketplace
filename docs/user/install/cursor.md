...
it could never have worked. The public changelog's latest *feature* number may lag the CLI
patch line (e.g. changelog **3.11** vs CLI **3.14.7**).
...
the plugins in their in-editor marketplace panel. Developers install from **Customize**
(3.9+) or the marketplace panel.
...
4. Save, then assign distribution groups — **organization groups** are supported since
   Cursor **3.10** (in addition to team-level SCIM directory groups)
5. Optional: enable **Auto Refresh** so pushes re-read the whole manifest

Developers: open **Customize** → your team marketplace → install the plugins you want.
...
### Team MCPs in the same marketplace (3.10+)

Admins can configure **Team MCP** servers once under Dashboard → Integrations & MCP and
make those same servers available in a team marketplace. That lets members install approved
integrations (for example a shared Production Master HTTP MCP) without hand-editing
`.cursor/mcp.json`. This catalog continues to list **plugins** only; Team MCPs are an
admin-side companion surface that can ride alongside the same marketplace.

First-party Cursor Marketplace plugins for **Google Drive / Gmail / Calendar** (2026-08-03)
are separate from this catalog — install them from Customize when a workflow needs Workspace
context.
...
- **Customize → Plugins** (or Settings → Plugins) lists the installed plugins
...
| Import succeeds but no plugins appear | Assign distribution groups (org groups on 3.10+) — an imported marketplace with no groups reaches nobody. |
...
| Need a shared MCP, not a plugin | Use Team MCPs (3.10+) under Dashboard → Integrations & MCP, optionally published into the same team marketplace. |
