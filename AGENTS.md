...
| `make validate-skills` | Native `claude plugin validate --strict --json` on `.agents/skills` (2.1.233+ frontmatter check, 2.1.259+ JSON report), summarized by `scripts/report-skill-validation.py`; soft-skips if the CLI isn't installed locally. CI runs it for real in the `skill-validate` job — see `docs/agent-guidelines/testing.md` |
